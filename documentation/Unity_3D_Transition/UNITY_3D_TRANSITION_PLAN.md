...
*   **Map Size:** The game utilizes a fixed, static boundary of **11x11 Chunks** (352x352 meters total space).
*   **Generation Strategy:** Procedural logic is discarded for horizontal layouts. We use Editor-level manual Prefab placement to guarantee artistic control, wrapping the result into a giant Floor Prefab which we vertically stack at runtime.
...
Focus shifts to establishing the 3D Hotel geometry.

1.  **Prefab-driven Level Design**
    *   Assemble the environment using manual placement of 32x32m chunks to form a single 11x11 floor layout.
    *   Group the layout into an overarching `Floor_01` prefab that can be seamlessly stacked.
